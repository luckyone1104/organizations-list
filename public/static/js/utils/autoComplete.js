function autoCompleteHover() {
  let autoCompleteElements = document.querySelectorAll(".autoComplete");

  autoCompleteElements.forEach((autoCompleteElement) => {
    autoCompleteElement.addEventListener("input", (event) => {
      setTimeout(() => {
        let autoCompleteResults = document.querySelectorAll(".autoComplete_result");

        for (let i = 0; i < autoCompleteResults.length; i++) {
          autoCompleteResults[i].addEventListener("mouseenter", function (event) {
            styleAutoCompleteResultOnMouseenter(event.target);
          });
          autoCompleteResults[i].addEventListener("mouseleave", function (event) {
            styleAutoCompleteResultOnMouseleave(event.target);
          });
        }

        function styleAutoCompleteResultOnMouseenter(autoCompleteResult) {
          let selectedText = autoCompleteResult.querySelector("span");
          autoCompleteResult.style.color = "#3e92b6";
          selectedText.style.color = "#4085a3";
        }
        function styleAutoCompleteResultOnMouseleave(autoCompleteResult) {
          let selectedText = autoCompleteResult.querySelector("span");
          autoCompleteResult.removeAttribute("style");
...
(function (global, factory) {
  ...
  var navigation = function (config, dataFeedback) {
    ...
    var addActive = function addActive(list) {
      ...
      if (classList)
        (_list$cursor$classLis = list[cursor].classList).add.apply(
          _list$cursor$classLis,
  ...
  var searchEngine = function (query, record, config) {
    ...
    if (config.searchEngine === "loose") {
      ...
      if (formattedRecord.includes(query)) {
        ...
        var _match = config.resultItem.highlight.render
